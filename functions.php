...
add_action( 'init', function () {
	...
	if ( post_type_exists( 'projects' ) ) {
		require_once get_template_directory() .
		             '/inc/template-builder/project-builder.php';
	}
...
/**
 * Retrieve existing ACF layout data for a given page and layout.
 */
function mcc_get_existing_layout_data($page_id, $layout) {
    $all_hero = get_field('hero_content_blocks', $page_id) ?: [];
    $all_flexi = get_field('flexible_content_blocks', $page_id) ?: [];

    foreach (array_merge($all_hero, $all_flexi) as $block) {
...
/**
 * HP POSTS AND PROJECTS FILTER
 */
// Generic archive counter: counts posts by post_type + taxonomy term
add_action('wp_ajax_hp_count_archive', 'hp_count_archive');
add_action('wp_ajax_nopriv_hp_count_archive', 'hp_count_archive');
function hp_count_archive() {
  check_ajax_referer('hp_archive_count', 'nonce');
  ...
  if ($term !== 'all' && taxonomy_exists($taxonomy)) {
  ...
  $q = new WP_Query($args);
  wp_send_json_success(array('count' => (int) $q->found_posts));
}
